...
class ExchangeRateClient:
    ...
    def convert(
        ...
        if source not in FIAT_CURRENCIES or target not in FIAT_CURRENCIES:
            raise ExchangeRateError(
                f"Cannot convert {source_currency.upper()} to {target_currency.upper()}"
            )
        return (amount * self.rate(source, target)).quantize(
    ...
    def rate(self, source_currency: str, target_currency: str) -> Decimal:
        ...
        request = Request(
            url, headers={"User-Agent": APP_USER_AGENT, "Accept": "application/json"}
        )
        try:
            ...
            raise ExchangeRateError(
                f"Exchange rate API returned HTTP {exc.code} for {url}: {detail}"
            ) from exc
        except URLError as exc:
            raise ExchangeRateError(
                f"Could not reach exchange rate API at {url}: {exc.reason}"
            ) from exc
    ...
    if isinstance(payload, list):
        for item in payload:
            if not isinstance(item, dict):
                continue
            if (
                str(item.get("base", "")).upper() == source
                and str(item.get("quote", "")).upper() == target
            ):
