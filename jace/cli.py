...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--database-url", help="Postgres connection URL. Defaults to DATABASE_URL."
    )

    subparsers = parser.add_subparsers(dest="command", required=True)

    track = subparsers.add_parser(
        "track", help="Fetch current prices and store a snapshot"
    )
    track.add_argument(
        "--database-url", default=argparse.SUPPRESS, help="Postgres connection URL"
    )
    ...
    track.add_argument(
        ...
        help="Scryfall price currency",
    )

    report = subparsers.add_parser(
        "report", help="Print latest prices and change since first snapshot"
    )
    report.add_argument(
        "--database-url", default=argparse.SUPPRESS, help="Postgres connection URL"
    )
...
def report(args: argparse.Namespace, store: PriceStore) -> int:
    ...
    print("-" * 78)
    for row in rows:
        name, set_code, number, quantity, currency, latest, first, change, latest_at = (
            _report_values(row)
        )
        ...
        print(
            f"{name} ({set_code} #{number}) | {quantity} | {latest_display} | {first_display} | {change_display} | {latest_at}"
        )
    return 0
...
def _report_values(
    ...
    str, str, str, int, str, Decimal | None, Decimal | None, Decimal | None, str
]:
