...
DEFAULT_MAX_IMPORT_CARDS = 10000
...
class AppConfig:
    ...
    scryfall_collection_request_interval_seconds: float = (
...
def app_config() -> AppConfig:
    ...
    if bool(auth_username) != bool(auth_password):
        raise ValueError(
            "JACE_AUTH_USERNAME and JACE_AUTH_PASSWORD must be set together"
        )
    return AppConfig(
        ...
        refresh_interval_seconds=env_int(
            "JACE_REFRESH_INTERVAL_SECONDS", DEFAULT_REFRESH_INTERVAL_SECONDS, minimum=1
        ),
        scryfall_base_url=env_str("JACE_SCRYFALL_BASE_URL", DEFAULT_SCRYFALL_BASE_URL),
        scryfall_bulk_size=env_int(
            "JACE_SCRYFALL_BULK_SIZE", DEFAULT_SCRYFALL_BULK_SIZE, minimum=1, maximum=75
        ),
        ...
        scryfall_timeout_seconds=env_float(
            "JACE_SCRYFALL_TIMEOUT_SECONDS",
            DEFAULT_SCRYFALL_TIMEOUT_SECONDS,
            minimum=0.1,
        ),
        image_fetch_timeout_seconds=env_float(
            "JACE_IMAGE_FETCH_TIMEOUT_SECONDS",
        ...
        max_request_body_bytes=env_int(
            "JACE_MAX_REQUEST_BODY_BYTES", DEFAULT_MAX_REQUEST_BODY_BYTES, minimum=1024
        ),
        max_import_cards=env_int(
            "JACE_MAX_IMPORT_CARDS", DEFAULT_MAX_IMPORT_CARDS, minimum=1
        ),
        max_import_jobs=env_int(
            "JACE_MAX_IMPORT_JOBS", DEFAULT_MAX_IMPORT_JOBS, minimum=1
        ),
        max_image_bytes=env_int(
            "JACE_MAX_IMAGE_BYTES", DEFAULT_MAX_IMAGE_BYTES, minimum=1024
        ),
...
def env_currency(name: str, default: str) -> str:
    ...
    if value not in SUPPORTED_CURRENCIES:
        raise ValueError(
            f"{name} must be one of {', '.join(sorted(SUPPORTED_CURRENCIES))}"
        )
    return value
